...
Vue.use(vee_validate__WEBPACK_IMPORTED_MODULE_0__["default"]); // Todo - auto fill from other forms
  ...
  methods: {
    submitForm: function submitForm() {
      ...
      this.$validator.validateAll().then(function (result) {
        if (result) {
          if (!_this.termsChecked) {
            toastr.error('Please make sure you have checked the terms and conditions.');
            return;
          }

          var data = {
            'orderDetails': _.clone(orderDetails.$data)
          };
          axios.post('/api/orderDetails', data).then(function () {
            window.location.href = location.protocol + "//" + location.host + "/order/overview";
          })["catch"](function () {
            return toastr.error('Unfortunately there\'s been a problem adding your order details. Please check your internet connection, refresh your page and try again or, alternatively, phone us on ' + window.sitePhone + ', and we\'ll try to help.');
          });
        } else {
    ...
    createValidationCustomMessages: function createValidationCustomMessages() {
      var dict = {
        custom: {
